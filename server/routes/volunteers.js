...
router.get('/code/:code', authenticate, requireManager, async (req, res) => {
  const user = await store.getUserByCode(req.params.code.toUpperCase());
...
router.put('/me/skills', authenticate, async (req, res) => {
  const { skills } = req.body;
  if (!Array.isArray(skills)) return res.status(400).json({ error: 'skills must be an array' });
  const updated = await store.updateUser(req.user.id, { skills });
